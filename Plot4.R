...
##      PLOT #4 Histogram of Global Active Power: plot4.png
plot.new()
dev.cur()
dev.copy (png, file ="./ExplAnalData/plot4.png")
#multiple plots: http://www.statmethods.net/advgraphs/layout.html
par(mfrow = c(2,2), cex=0.65)

# SubPlot Top left: same as plot#2
plot(hpc.sub$DateTime, hpc.sub$Global_active_power, xlab="",ylab="Global Active Power", type="l")

# Subplot Bottoum left: same as plot#3
plot(hpc.sub$DateTime, hpc.sub$Voltage, xlab="datetime",ylab="Voltage", type="l")

# Subplot Top right:
plot(hpc.sub$DateTime, hpc.sub$Sub_metering_1, xlab="",ylab="Energy Sub metering", type="l")
lines(hpc.sub$DateTime, hpc.sub$Sub_metering_2, col="red", type="l")
lines(hpc.sub$DateTime, hpc.sub$Sub_metering_3, col="blue",type="l")

# Subplot Bottom right:
plot(hpc.sub$DateTime, hpc.sub$Global_reactive_power, xlab="datetime",ylab="Global Reactive Power", type="l")
dev.off()